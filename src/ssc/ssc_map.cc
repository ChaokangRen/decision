...
// RCK
// tips:该函数为核心函数，主要用于构建自车轨迹的语义走廊，具体实现方法需要看相关文献。
    ...
    for (int i = 0; i < seed_num; ++i) {
        if (i == 0) {
            ...
            if (CheckIfCubeContainsSeed(driving_corridor.cubes.back().cube,
                ...
                if (!CheckIfCubeIsFree(p_grid, cube)) {
                    sprintf_error(
                        "[Ssc] SccMap - Initial cube is not free, seed id: %d",
                        i);
                    common::DrivingCube driving_cube;
                    driving_cube.cube = cube;
                    driving_cube.seeds.push_back(traj_seeds[i]);
...
ErrorType SscMap::InflateCubeIn3dGrid(GridMap3D *p_grid,
    ...
    int x_p_step = dir_step[0];  // 20
    int x_n_step = dir_step[1];  // 5
    int y_p_step = dir_step[2];  // 10
    int y_n_step = dir_step[3];  // 10
            ...
            for (int k = 0; k < static_cast<int>(corridor.cubes.size()); ++k) {
                ...
                if (k == 0) {
                    if (y_lb > initial_fs_.vec_dt[0] ||
                        ...
                        sprintf_error(
                            "[Ssc] SscMap - Initial state out of bound d: %f, "
                            "lb: %f,ub: %f",
                            initial_fs_.vec_dt[0], y_lb, y_ub);
                        return kWrongStatus;
                    }
                }

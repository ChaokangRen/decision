...
ErrorType SplineGenerator<N_DEG, N_DIM>::GetCubicSplineBySampleInterpolation(
    ...
    SplineType *spline) {
    if (samples.size() < 3) {
        // sprintf("[SampleInterpolation]input sample size: %d.\n",
...
ErrorType SplineGenerator<N_DEG, N_DIM>::GetQuinticSplineBySampleFitting(
    ...
    if (N_DEG < 5) {
        sprintf("[QuinticSpline]Degree not support.\n");
        return kWrongStatus;
    }
    if (breaks.size() < 2) {
        sprintf("[QuinticSpline]Cannot support less than two segment.\n");
        return kWrongStatus;
    }

    if (samples.size() < 3) {
        // sprintf("[QuinticSpline]Cannot support less than three samples.\n");
        return kWrongStatus;
    }
    ...
    // sprintf("time consumed in wrap constraints: %lf.\n", t_elapsed);
...
ErrorType SplineGenerator<N_DEG, N_DIM>::GetSplineFromStateVec(
    ...
    if (para.size() < 2) {
        sprintf(
            "[GetSplineFromStateVec]failed to get spline from state vec.\n");
        return kWrongStatus;
    }
...
ErrorType SplineGenerator<N_DEG, N_DIM>::GetBezierSplineUsingCorridor(
    ...
    if (!OoQpItf::solve(Q, c, A, b, C, lbd, ubd, l, u, x, true, false)) {
        sprintf("[GetBezierSplineUsingCorridor]Solver error.\n");
        return kWrongStatus;
    }

...
ErrorType StateTransformer::GetStateFromFrenetState(const FrenetState &fs,
                                                    State *s) const {
    if (!lane_.IsValid()) {
        sprintf("[StateFromFrenetState]Err: lane not valid.\n");
    ...
    if (LaneDim != 2) {
        sprintf("[StateFromFrenetState]Err: cannot support non-plane now.\n");
        return kIllegalInput;
    }
...
ErrorType StateTransformer::GetFrenetStateFromState(const State &s,
    ...
    //   // sprintf("[FrenetStateFromState]State angle %lf not on lane
    //   (%lf).\n",
    ...
    if (one_minus_curd < kEPS) {
        // sprintf("[StateFromFrenetState]d not valid for transform.\n");
        return kWrongStatus;
    }
